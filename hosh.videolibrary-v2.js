...
function classifyVideo(title, description, DRs){
	...
	for (key in categories){
		if (searchDescription(key)){
			classify += categories[key]
		}
	}
	...
	function searchDescription(tagTerm){
		tag = new RegExp( tagTerm, 'i')
		if (content.search(tag) > -1){
			return true
		}
	}
}
